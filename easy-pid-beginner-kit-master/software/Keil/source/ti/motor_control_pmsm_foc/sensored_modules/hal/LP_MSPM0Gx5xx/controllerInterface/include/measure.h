...
/*! CAPTURE TIMER CLOCK FREQUENCY*/
#define TIMER_CAPT_CLK_FREQ             80000000U
...
/*! @brief Measure Hall Index  */
typedef struct
{
    HALL_INTERFACE_T
        /*! Hall Interface Handle */
        HallInterface;
    uint8_t
        /*! Hall state*/
        hallState;
    uint32_t
        /*! Hall Edge to Edge Timer Counts*/
        commCountIndex[MAX_HALL_INDEX],
        /*! Hall Edge to Edge Timer Running Counts*/
        commTimerIndex[MAX_HALL_INDEX];
    int32_t
        /*! Hall Edge Frequency*/
        halCapFreqPU;
    uint32_t
        /*! Hall Max Frequency Counts*/
        halMaxFreqCount;
    uint8_t
    /*! Complimentary Hall State*/
        compHallState;
    /*! Count Diff*/
    uint32_t
        countDiff;
...
/**
 * @brief     ReadHallState
 * @param[in] *pHallHandle Input Pointer
 * @return Hall State
 */
uint16_t ReadHallState(HAL_MEASURE_HALL_HANDLE_T *pHallHandle);
/**
 * @brief    getHallFrequency
 * @param[in] *pHallHandle Input Pointer
 * @return Motor Speed in Electrical Hertz
 */
