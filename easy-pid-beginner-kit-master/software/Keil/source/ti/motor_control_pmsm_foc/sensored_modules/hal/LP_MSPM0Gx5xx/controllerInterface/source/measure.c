...
void inputsInit(HAL_MEASURE_MOTOR_INPUTS_T *pMotorInputs)
{
    ...
    pMotorInputs->measureHALL.HallInterface.hallA = HAL_GPIO_IN_01;
    pMotorInputs->measureHALL.HallInterface.hallB = HAL_GPIO_IN_02;
    pMotorInputs->measureHALL.HallInterface.hallC = HAL_GPIO_IN_03;

    pMotorInputs->measureHALL.HallInterface.captureInput = HAL_CAPTURE_TIMER_01;

    pMotorInputs->measureHALL.halMaxFreqCount = _IQ8div(TIMER_CAPT_CLK_FREQ,MOTOR_MAX_SPEED);
}
...
{
    pHallHandle->hallState = (HAL_readGPIOPin(pHallHandle->HallInterface.hallA) +
                            (HAL_readGPIOPin(pHallHandle->HallInterface.hallB) <<1) +
                            (HAL_readGPIOPin(pHallHandle->HallInterface.hallC) << 2));
    return pHallHandle->hallState;
}
int32_t getHallFrequency(HAL_MEASURE_HALL_HANDLE_T *pHallHandle)
{
    pHallHandle->commCountIndex[pHallHandle->hallState] = HAL_getCaptureValue(pHallHandle->HallInterface.captureInput);

    pHallHandle->commTimerIndex[pHallHandle->hallState] = HAL_getCaptureRunningValue(pHallHandle->HallInterface.captureInput);

    pHallHandle->compHallState = MAX_HALL_INDEX - pHallHandle->hallState;

    pHallHandle->countDiff = (pHallHandle->commCountIndex[pHallHandle->compHallState] > pHallHandle->commCountIndex[pHallHandle->hallState])?
            (pHallHandle->commCountIndex[pHallHandle->compHallState] - pHallHandle->commCountIndex[pHallHandle->hallState]) :
            (HAL_getTimerLoadValue(pHallHandle->HallInterface.captureInput) + (pHallHandle->commCountIndex[pHallHandle->compHallState] - pHallHandle->commCountIndex[pHallHandle->hallState]));

    // HalMaxFreqCount is in IQ8 format, Counts are IQ-1 bit Format(Counts for Hal edge to be Multiplied by 2) - the resultant Speed in PU is IQ27(Global) Hence 27 - (8-1)= 20
    pHallHandle->halCapFreqPU =  _IQ18div_mathacl(pHallHandle->halMaxFreqCount,pHallHandle->countDiff);

    pHallHandle->countError = (pHallHandle->commCountIndex[pHallHandle->hallState] > pHallHandle->commTimerIndex[pHallHandle->hallState])?
            (pHallHandle->commCountIndex[pHallHandle->hallState] - pHallHandle->commTimerIndex[pHallHandle->hallState]) :
            (HAL_getTimerLoadValue(pHallHandle->HallInterface.captureInput) + (pHallHandle->commCountIndex[pHallHandle->hallState] - pHallHandle->commTimerIndex[pHallHandle->hallState]));

    return pHallHandle->halCapFreqPU;
}
