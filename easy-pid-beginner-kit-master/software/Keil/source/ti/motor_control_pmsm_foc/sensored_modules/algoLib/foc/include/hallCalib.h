...
#ifndef FOC_INCLUDE_HALL_CALIBRATION_H_
#define FOC_INCLUDE_HALL_CALIBRATION_H_

#include "stdint.h"
#include "mcLib.h"
#include "measure.h"

/*! @brief ALIGN ANGLE FORWARD - HALL CALIBRATION
 * Applied Voltage Vector from SVM is 90 Degree Phase advanced from
 * Aligned Rotor Position
 * During Align, compensate the offset */
#define CALIBRATION_ALIGN_ANGLE  _IQ(0.125)


/*! @brief CALIBRATION ANGLE STEP SIZE */
#define CALIBRATION_ANGLE_STEP  _IQ(0.5/360.0)
/*! @brief HALL angle mask to reset for 27 bits*/
#define HALL_ANGLE_MASK  0x7FFFFFF
/*! @brief Hall angle Overflow Mask for 27 bits */
#define HALL_ANGLE_OVERFLOW_MASK  0xF8000000

/*! @brief HallCalibration State */
typedef enum
{
...
/*! @brief Hall Calibration Working Structure */
typedef struct
{
    /*! Hall Calibration Inputs */
    ...
    /*! Align Angle */
        angle;
       int32_t
   /*! Hall Angle in PU Forward */
       *pThetaHallForwardPU;

       int32_t
   /*! Hall Angle in PU Reverse*/
       *pThetaHallReversePU;
    ...
    /*! Maximum count value for align on stop */
        countMaxStop,
    /*! Maximum count value for Calib Run */
        countMaxCalibRun,
    /*! counter for Calib Sector Run */
        calibSectorCount,
    /*! Hall Index */
        hallIndex,
    /*! Hall Index Prev*/
    ...
    HALL_CALIBRATION_STATE_e
    /*! Hall Calibration State */
        calibState;


    /*! Hall Calibration Status */
    bool
        status;
    bool
    /*! Hall Calibration Enable */

        hallCalibEnable;
    /*! pointer for Number of Hall Sectors per Rev */
    const uint8_t *pHallSectorPerRev;

    /*! Pointer Handle for Hall Interface  */
    HAL_MEASURE_HALL_HANDLE_T *pHALLIntHandle;

}HALL_CALIB_T;

/**
 * @brief     Hall Calib Init
 * @param[in] *pHallCalibHandle Input Pointer
 */
void hallCalibInit(HALL_CALIB_T *pHallCalibHandle);
/**
 * @brief    Hall Calib Run
 * @param[in] *pHallCalibHandle Input Pointer
 */
void hallCalibRun(HALL_CALIB_T *pHallCalibHandle);
/**
 * @brief    Get Calib Status
 * @param[in] *pHallCalibHandle Pointer
 * @return    Returns the status
 */
inline _Bool getHallCalibStatus(HALL_CALIB_T *pHallCalibHandle)
{
    return pHallCalibHandle->status;
}

#endif /* FOC_INCLUDE_HALL_CALIBRATION_H_ */
