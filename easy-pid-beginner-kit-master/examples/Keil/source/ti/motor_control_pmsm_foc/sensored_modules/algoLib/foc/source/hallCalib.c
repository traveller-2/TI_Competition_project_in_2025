...
void hallCalibInit(HALL_CALIB_T *pHallHandle)
{
    pHallHandle->count = 0;

    pHallHandle->calibState = HALL_CALIB_INIT;

    pHallHandle->iqRef = 0;

    pHallHandle->idRef = 0;

    pHallHandle->status = FALSE;

    pHallHandle->calibSectorCount = 0;

}

void hallCalibRun(HALL_CALIB_T *pHallHandle)
{
    int32_t targetVelocity = *(pHallHandle->pTargetVelocity);

    static int32_t

    /*! Hall Angle Error in PU*/
    hallAngleComputeErrorForward = 0,
    /*! Hall Angle Offset */
    hallAngleComputeOffsetForward = 0,
    /*! Hall Angle Error in PU*/
    hallAngleComputeErrorReverse = 0,
    /*! Hall Angle Offset */
    hallAngleComputeOffsetReverse = 0;

    switch(pHallHandle->calibState)
    {
    case HALL_CALIB_INIT:

        pHallHandle->calibState = HAL_CALIB_ALIGN;
        pHallHandle->angle = CALIBRATION_ALIGN_ANGLE;
        ...
        for(uint16_t hallIndex = 1; hallIndex < MAX_HALL_INDEX ; hallIndex ++)
        {
            *(pHallHandle->pThetaHallForwardPU + hallIndex) = 0;

            *(pHallHandle->pThetaHallReversePU + hallIndex) = 0;
        }
        break;

    case HAL_CALIB_ALIGN:

        if(pHallHandle->count >= pHallHandle->countMaxAlign)
        {
            ...
            pHallHandle->count = 0;
            pHallHandle->calibState = HAL_CALIB_RUN_FORWARD;
        }

        break;
    case HAL_CALIB_RUN_FORWARD:
        ...
        {
                if(pHallHandle->calibSectorCount < *pHallHandle->pHallSectorPerRev)
                {
                    *(pHallHandle->pThetaHallForwardPU + pHallHandle->hallIndex)+=

                            (pHallHandle->angle & HALL_ANGLE_MASK);
                    pHallHandle->calibSectorCount++;
                }
                else
                {
                    pHallHandle->calibSectorCount = 0;
                    pHallHandle->calibState = HAL_CALIB_RUN_REVERSE;
                }
        }
        // Increment Angle by Defined Angle Step for 1 Mechanical cycle in Forward direction.

        if(pHallHandle->count >= pHallHandle->countMaxCalibRun)
        {
            pHallHandle->count = 0;
            pHallHandle->angle += CALIBRATION_ANGLE_STEP;
        }
        break;

    case HAL_CALIB_RUN_REVERSE:
            ...
            if(pHallHandle->hallIndex != pHallHandle->hallIndexPrev)
            {
                    if(pHallHandle->calibSectorCount < *pHallHandle->pHallSectorPerRev)
                    {
                        *(pHallHandle->pThetaHallReversePU +pHallHandle->hallIndex)+=
                    ...
                    {
                        pHallHandle->calibSectorCount = 0;
                        pHallHandle->calibState = HAL_CALIB_COMPUTE;
                    }
            }
            // Decrement Angle by Defined Angle Step for 1 Mechanical cycle in reverse Direction.
    ...
    case HAL_CALIB_COMPUTE:

       for(uint16_t hallIndex = 1; hallIndex < MAX_HALL_INDEX ; hallIndex ++)
       {
           *(pHallHandle->pThetaHallForwardPU + hallIndex) /= POLE_PAIRS;

           *(pHallHandle->pThetaHallReversePU + hallIndex) /= POLE_PAIRS;

           /* If any of the calibrated Angles cross overs 360 degrees, trigger HALL_CALIB_FAILED */
           if((*(pHallHandle->pThetaHallForwardPU + hallIndex) > _IQ(1.0)) || (*(pHallHandle->pThetaHallReversePU + hallIndex) > _IQ(1.0)))
           {
               pHallHandle->calibState = HAL_CALIB_FAILED;
           }
       }
       ...
       for(uint16_t hallIndex = 1; hallIndex < MAX_HALL_INDEX ; hallIndex ++)
       {
           hallAngleComputeErrorForward += *(pHallHandle->pThetaHallForwardPU + hallIndex);

           hallAngleComputeErrorReverse += *(pHallHandle->pThetaHallReversePU + hallIndex);
       }

       hallAngleComputeErrorForward =  HALL_ANGLE_SUM - hallAngleComputeErrorForward;

       hallAngleComputeErrorReverse = HALL_ANGLE_SUM - hallAngleComputeErrorReverse;

       hallAngleComputeOffsetForward = hallAngleComputeErrorForward / (MAX_HALL_INDEX - 1) ;

       hallAngleComputeOffsetReverse = hallAngleComputeErrorReverse / (MAX_HALL_INDEX - 1) ;


       for(uint16_t hallIndex = 1; hallIndex < MAX_HALL_INDEX ; hallIndex ++)
       {
           *(pHallHandle->pThetaHallForwardPU + hallIndex) +=  hallAngleComputeOffsetForward;

           *(pHallHandle->pThetaHallReversePU + hallIndex) +=  hallAngleComputeOffsetReverse;
       }

       /* If Calibration is successful sitch to Calibration complete */
       if(pHallHandle->calibState != HAL_CALIB_FAILED)
           pHallHandle->calibState = HAL_CALIB_COMPLETE;

       break;
    case HAL_CALIB_COMPLETE:
        pHallHandle->status = TRUE;
        break;
    case HAL_CALIB_FAILED:
        pHallHandle->status = TRUE;
        break;
    default:
        break;
