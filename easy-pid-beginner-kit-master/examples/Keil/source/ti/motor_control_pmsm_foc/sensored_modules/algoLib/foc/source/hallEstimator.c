...
#define HAL_VEL_FILT_CONST                             0.1
#define HAL_VEL_FILT_CONST_FACTOR                      _IQ(0.628318530718)
#define HAL_TWO_PI                                     _IQ(6.28318530718)
#define HALL_SECTOR_ZERO                               0
#define HALL_SECTOR_SEVEN                              7

void hallAngleInit(HALL_ESTIMATOR_OBJ_T *pHallHandle)
{
    pHallHandle->hallIndex = ReadHallState(pHallHandle->pHALLIntHandle);
    pHallHandle->hallSectorCount = 0;
    pHallHandle->hallEstimFluxAngle =  *(pHallHandle->pThetaHallIndexPU + pHallHandle->hallIndex);
    pHallHandle->gainEst        = _IQ(1.0);
}
int32_t hallForcedAngleRun(HALL_ESTIMATOR_OBJ_T *pHallHandle)
{
    return( *(pHallHandle->pThetaHallIndexPU + pHallHandle->hallIndex));
}
bool hallInvalidStallDetectRun(uint16_t *pHallSector)
{
    return((*pHallSector == HALL_SECTOR_ZERO) || (*pHallSector == HALL_SECTOR_SEVEN));
}
void hallAngleEstimRun(HALL_ESTIMATOR_OBJ_T *pHallHandle)
{
    int32_t hallElecSpeed;
    pHallHandle->hallIndexPrev = pHallHandle->hallIndex;

    pHallHandle->hallIndex = ReadHallState(pHallHandle->pHALLIntHandle);



    pHallHandle->hallSectorChange = FALSE;

    if(pHallHandle->hallIndex != pHallHandle->hallIndexPrev)
    {
        pHallHandle->hallSpeedPWMCount = 1;
        pHallHandle->hallIndexNext[pHallHandle->hallIndexPrev] =  pHallHandle->hallIndex;

        pHallHandle->hallSectorCount++;
        pHallHandle->hallSectorChange = TRUE;

        hallElecSpeed = getHallFrequency(pHallHandle->pHALLIntHandle);

        if(*pHallHandle->pTargetVelocity < 0)
        {
            pHallHandle->hallEstimVelocity = -hallElecSpeed;
        }
        else
        {
            pHallHandle->hallEstimVelocity = hallElecSpeed;
        ...
        pHallHandle->hallEstimFluxAngle =  (pHallHandle->hallEstimFluxAngle & HALL_ANGLE_OVERFLOW_MASK)
                                                + *(pHallHandle->pThetaHallIndexPU + pHallHandle->hallIndex);

        pHallHandle->hallNextIndexAngle =  *(pHallHandle->pThetaHallIndexPU + pHallHandle->hallIndexNext[pHallHandle->hallIndex]);

        pHallHandle->hallEstimFluxAngle += _IQmpy_mathacl(pHallHandle->hallEstimVelocity,
                                                                   pHallHandle->pHALLIntHandle->countError);

    }

    else
    {
        pHallHandle->freqTs =  _IQmpy_mathacl(pHallHandle->hallEstimVelocity,
                                       *pHallHandle->pDeltaT);

        if(_IQabs(pHallHandle->hallNextIndexAngle - (pHallHandle->hallEstimFluxAngle & HALL_ANGLE_MASK)) <= _IQabs(pHallHandle->freqTs))
        {
            pHallHandle->hallSpeedPWMRatio = _IQ18div_mathacl(_IQ18(pHallHandle->hallSpeedPWMCount),_IQ18(pHallHandle->hallSpeedPWMCount+1));
            pHallHandle->hallEstimVelocity = _IQ18mpy_mathacl(pHallHandle->hallEstimVelocity,pHallHandle->hallSpeedPWMRatio);
            pHallHandle->hallEstimFluxAngle = pHallHandle->hallNextIndexAngle + (pHallHandle->hallEstimFluxAngle & HALL_ANGLE_OVERFLOW_MASK);
        }
        else
        {
            pHallHandle->hallEstimFluxAngle += pHallHandle->freqTs;
        }
        pHallHandle->hallSpeedPWMCount++;
...
//    pHallHandle->freqTs = _IQmpy_mathacl(_IQabs(pHallHandle->hallEstimVelocity),
//                     *pHallHandle->pDeltaT);
//
//    temp = _IQmpy_mathacl(pHallHandle->freqTs, HAL_VEL_FILT_CONST_FACTOR);
//
//    pHallHandle->hallVelFiltConst = _IQmpy_mathacl(temp, (_IQ(1.0) - temp));
//
//    filter_mathacl(&(pHallHandle->hallEstimVelocityFilt),
//                   pHallHandle->hallEstimVelocity,pHallHandle->hallVelFiltConst);
//
//    pHallHandle->hallEstimSpeedFilt = _IQabs(pHallHandle->hallEstimVelocityFilt);

    pHallHandle->hallEstimSpeedFilt = pHallHandle->hallEstimVelocity;

    pHallHandle->hallEstimVelocityFilt = pHallHandle->hallEstimVelocity;
}

void hallSinCosEstim(MC_SINCOS_T *pFOCSinCos ,HALL_ESTIMATOR_OBJ_T *pHallHandle)
{
    _iq Err = _IQmpy_mathacl(((pFOCSinCos->sin) - pHallHandle->sinCos.sin),(pHallHandle->gainEst));

       pHallHandle->omegaTs = _IQmpy_mathacl(pHallHandle->freqTs,HAL_TWO_PI);

       _iq In1 = Err + pHallHandle->sinCos.cos;

       pHallHandle->prevSinCos.sin = pHallHandle->sinCos.sin;

       pHallHandle->sinCos.sin = pHallHandle->prevSinCos.sin +  ( _IQmpy_mathacl((In1 + pHallHandle->sinErr), pHallHandle->omegaTs) >> 1);

       pHallHandle->sinErr = In1;

       pHallHandle->sinCos.cos = -((-pHallHandle->sinCos.cos) + ( _IQmpy_mathacl((pHallHandle->sinCos.sin + pHallHandle->prevSinCos.sin),pHallHandle->omegaTs) >> 1));
