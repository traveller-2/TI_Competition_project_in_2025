...
#include "hallCalib.h"
 ...
 /*! @brief Min target Duty Float */
#define MIN_TARGET_DUTY_FLOAT                       0.01
 /*! @brief Min Target Duty */
#define MIN_TARGET_SPEED                            _IQ(MIN_TARGET_DUTY_FLOAT)

 /*! @brief Voltage Out of Bound Deglitch Time in mS */
#define VOLTAGE_OUT_OF_BOUNDS_TIME_MSEC             50.0
 /*! @brief No Motor Fault Detect Deglitch Time in mS*/
#define NO_MOTOR_TIME_MSEC                         500
 /*! @brief VAbnormal Speed Fault Detect Deglitch Time in mS */
...
typedef struct
{
    ...
    DEAD_TIME_COMPUTE_T
    /*! Dead Time Compensation Structure */
        deadTime;
    ...
    int32_t
    /*! Hall Angle Table Forward*/
        hallAngleTableForward[MAX_HALL_INDEX],
    /*! Hall Angle Table Reverse*/
        hallAngleTableReverse[MAX_HALL_INDEX];

}SENSORED_FOC_APPLICATION_T;

/*! @brief Motor Control Application Variable  */
extern SENSORED_FOC_APPLICATION_T *g_pMC_App;
