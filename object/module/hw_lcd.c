...
#define CHAR_WIDTH 8     // 字体宽度，英文通常8像素
#define CHAR_HEIGHT 16   // 字体高度
 ...
 * @param y 起始Y坐标（矩形左上角）
 * @param str 要显示的字符串
 * @param max_chars 显示区域最大字符数（用于计算宽度）
 */
void LCD_ShowStr(int x, int y, const char *str, int max_chars)
{
    int str_len = strlen(str);
    int area_w = max_chars * CHAR_WIDTH;
    int area_h = CHAR_HEIGHT;

    int str_w = str_len * CHAR_WIDTH;
    int start_x = x + (area_w - str_w) / 2;
    int start_y = y;

    // 黑底擦除原区域
    LCD_Fill(x, y, x + area_w, y + area_h, BLACK);

    // 显示字符串（白色字体）
    LCD_ShowString(start_x, start_y, (uint8_t *)str, WHITE, BLACK, CHAR_HEIGHT, 0);
}
